...
class ConditionalWGAN(keras.Model):
    def __init__(self, discriminator, generator, latent_dim, image_size, num_classes, discriminator_extra_steps=3,
        gp_weight=10.0):
        super().__init__()
    ...
    def from_config(cls, config):
        config["discriminator"] = tf.keras.utils.deserialize_keras_object(config["discriminator"])
        config["generator"] = tf.keras.utils.deserialize_keras_object(config["generator"])
        config["latent_dim"] = tf.keras.utils.deserialize_keras_object(config["latent_dim"])
        config["discriminator_extra_steps"] = tf.keras.utils.deserialize_keras_object(config["discriminator_extra_steps"])
        return cls(**config)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            predicted_labels = self.discriminator(fake_image_and_labels, training=True)
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        one_hot_labels = tf.fill([self.num_img],value=1) # always generate images for class 1
        one_hot_labels = tf.one_hot(one_hot_labels, depth=self.num_classes)

        random_latent_vectors = keras.random.normal(
                shape=(self.num_img, self.latent_dim), seed=keras.random.SeedGenerator(1337))
        print("one hot label shape", one_hot_labels.shape, "random shape", random_latent_vectors.shape)
        random_vector_labels = ops.concatenate(
                [random_latent_vectors, one_hot_labels], axis=1)
        ...
        for i in range(self.num_img):
            ...
            image_path = os.path.join(self.image_dir,"generated_img_{i}_{epoch}.png")
